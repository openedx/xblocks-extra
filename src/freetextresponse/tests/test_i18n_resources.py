...
def mock_gettext(source, *_args):
    ...
    return {
        "Submit": "Süßmït",
        "Your response must be between {min} and {max} word.": "Ýöür réspönsé müst ßé ßétwéén {min} änd {max} wörd.",
        "Your response must be between {min} and {max} words.": "Ýöür réspönsé müst ßé ßétwéén {min} änd {max} wörds.",
    }.get(source, source)
...
class TestFreeTextResponseI18N(TestCase):
    ...
    def test_esperanto_translations_in_student_view(self):
        ...
        with (
            django.utils.translation.override("eo"),
