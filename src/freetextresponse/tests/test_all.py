...
class FreetextResponseXblockTestCase(unittest.TestCase):
    ...
    def test_generate_validation_message(self):
        ...
        result = ValidationMessage(ValidationMessage.ERROR, msg)
    ...
    def test_determine_credit(self, **test_data):
        ...
        """
        self.xblock._word_count_valid = MagicMock(return_value=test_data["word_count_valid"])
    ...
    def test_compute_score_integrity_error(self):
        ...
        """
        self.xblock.runtime.publish = MagicMock(return_value=None)
        self.xblock.runtime.publish.side_effect = IntegrityError("Unique Key Violation")
    ...
    def test_is_at_least_one_phrase_present(self):
        ...
        """
        keyphrases_list = ["do dict phrase", "re dict phrase", "mi dict phrase", "fa dict phrase"]
    ...
    @ddt.unpack
    def test_used_attempts_feedback_normal(self, max_attempts, count_attempts, result):
        # pylint: disable=invalid-name, protected-access
        """
    ...
    def test_get_user_alert(self, **test_data):
        ...
        self.assertEqual(
            ...
            self.xblock._get_user_alert(ignore_attempts=test_data["ignore_attempts"]),
        )
    ...
    def test_get_indicator_class(self, **test_data):
        ...
        self.xblock._word_count_valid = MagicMock(return_value=test_data["word_count_valid"])
        self.xblock._determine_credit = MagicMock(return_value=credit)
        self.assertEqual(test_data["result"], self.xblock._get_indicator_class())
    ...
    def test_get_submitdisplay_class(self, **test_data):
        ...
        self.xblock.count_attempts = test_data["count_attempts"]
        self.assertEqual(test_data["result"], self.xblock._get_nodisplay_class())
    ...
    @ddt.data(
        ...
        (
            {
                "due": dt.now(UTC).replace(tzinfo=None) + timedelta(hours=1),
                "graceperiod": None,
            },
            True,
        ),
        ...
        (
            {
                "due": dt.now(UTC).replace(tzinfo=None) - timedelta(hours=5),
                "graceperiod": timedelta(hours=2),
            },
            False,
        ),
    ...
    def test_submit(self, xblock_config, can_submit):
        ...
        self.assertEqual(response.json_body["problem_progress"], self.xblock._get_problem_progress())
        self.assertEqual(response.json_body["indicator_class"], self.xblock._get_indicator_class())
        self.assertEqual(response.json_body["used_attempts_feedback"], self.xblock._get_used_attempts_feedback())
        self.assertEqual(response.json_body["nodisplay_class"], self.xblock._get_nodisplay_class())
        self.assertEqual(response.json_body["submitted_message"], self.xblock._get_submitted_message())
        self.assertEqual(
        ...
        self.assertEqual(response.json_body["visibility_class"], self.xblock._get_indicator_visibility_class())
        self.assertEqual(self.xblock.student_answer, student_answer if can_submit else "")
    ...
    def test_save_response(self, xblock_config, can_save):
        ...
        request.method = "POST"
        request.body = data.encode("utf-8")
        response = self.xblock.save_response(request)
        # Added for response json_body
        # pylint: disable=no-member
        self.assertEqual(response.json_body["status"], "success")
        self.assertEqual(response.json_body["problem_progress"], self.xblock._get_problem_progress())
        ...
        self.assertEqual(response.json_body["used_attempts_feedback"], self.xblock._get_used_attempts_feedback())
        self.assertEqual(response.json_body["nodisplay_class"], self.xblock._get_nodisplay_class())
        self.assertEqual(response.json_body["submitted_message"], "")
        self.assertEqual(response.json_body["user_alert"], self.xblock.saved_message)
        self.assertEqual(response.json_body["visibility_class"], self.xblock._get_indicator_visibility_class())
        self.assertEqual(self.xblock.student_answer, student_answer if can_save else "")
