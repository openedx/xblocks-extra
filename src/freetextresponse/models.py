...
class FreeTextResponseModelMixin:
    """
    Handle data access for Free Text Response XBlock instances
    """
    ...
    display_other_student_responses = Boolean(
        ...
        help=_("This will display other student responses to the student after they submit their response."),
        default=False,
        scope=Scope.settings,
    )
    ...
    max_attempts = Integer(
        ...
        help=_("This is the maximum number of times a student is allowed to attempt the problem"),
        default=0,
        values={"min": 0},
        scope=Scope.settings,
    )
    max_word_count = Integer(
        ...
        help=_("This is the maximum number of words allowed for this question"),
        default=10000,
        values={"min": 1},
        scope=Scope.settings,
    )
    min_word_count = Integer(
        ...
        help=_("This is the minimum number of words required for this question"),
        default=1,
        values={"min": 1},
        scope=Scope.settings,
    )
    prompt = String(
        ...
        help=_("This is the prompt students will see when asked to enter their response"),
    ...
    submitted_message = String(
        ...
        help=_("This is the message students will see upon submitting their response"),
    ...
    saved_message = String(
        ...
        help=_("This is the message students will see upon submitting a draft response"),
        default=('Your answers have been saved but not graded. Click "Submit" to grade them.'),
        scope=Scope.settings,
    )
    ...
    def _compute_score(self):
        """
        Computes and publishes the user's score for the XBlock
        based on their answer
        """
        credit = self._determine_credit()
        self.score = credit.value
        try:
            self.runtime.publish(self, "grade", {"value": self.score, "max_value": Credit.full.value})
        except IntegrityError:
            log.warning("Failed to publish grade for block %s", self.scope_ids.usage_id, exc_info=True)
...
class Credit(Enum):
    ...
    awarded: Zero Credit, Half Credit, and Full Credit
    """
