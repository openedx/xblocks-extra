...
function FreeTextResponseView(runtime, element) {
    ...
    if (typeof $xblocksContainer.data(cachedAnswerId) !== 'undefined') {
        textareaStudentAnswer.val($xblocksContainer.data(cachedAnswerId));
    ...
    /**
     * Render peer responses into the list, escaping each answer to prevent XSS.
     * @param {Array} responses - a list of Responses
     * @returns {undefined} nothing
     */
    function renderStudentResponses(responses) {
        var noResponsesText = responseList.data('noresponse');
        responseList.empty();
        if (responses.length === 0) {
            responseList.append($('<li class="no-response"></li>').text(noResponsesText));
        } else {
            responses.forEach(function (item) {
                responseList.append($('<li class="other-student-responses"></li>').text(item.answer));
            });
        }
    }
    ...
    function displayResponsesIfAnswered(response) {
        ...
        renderStudentResponses(response.other_responses);
