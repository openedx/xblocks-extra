...
class SchoolYourselfReviewXBlockTest(unittest.TestCase):
    def setUp(self):
        ...
        self.block = SchoolYourselfReviewXBlock(
            self.mock_runtime, DictFieldData({}), ScopeIds("foo", "bar", "baz", "x")
        )
    ...
    def test_handle_grade_malformed_input(self):
        ...
        self.assertEqual(
            self.block.handle_grade_json(
                {"mastery": {"invalid_module_id": 1.0}, "user_id": "foo", "signature": "asdf"}
            ),
            "bad_request",
        )
    ...
    def test_handle_grade(self):
        ...
        self.assertEqual(
            self.block.handle_grade_json(
                {"mastery": {"algebra/multiplication": 0.7}, "user_id": "foo", "signature": "asdf"}
            ),
            "invalid_signature",
        )
        ...
        self.assertEqual(
            self.block.handle_grade_json(
                {"mastery": {"algebra/multiplication": 0.7}, "user_id": "foo", "signature": self.canned_signature}
            ),
            1.0,
        )

        self.mock_runtime.publish.assert_called_with(self.block, "grade", {"value": 1.0, "max_value": 1.0})

    def test_get_partner_url_params(self):
        # These are the defaults:
        self.assertEqual(self.block.get_partner_url_params(), {"partner": "edx", "partner_user_id": "debug"})

        self.block.partner_id = "foo"
        self.block.xmodule_runtime = FakeXModuleRuntime("abc123")

        self.assertEqual(self.block.get_partner_url_params(), {"partner": "foo", "partner_user_id": "abc123"})
